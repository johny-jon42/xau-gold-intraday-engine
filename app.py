...
from engine import analyze, news
from data import load_all

st.set_page_config(page_title='XAU/USD SMC Intraday Engine V10', page_icon='🥇', layout='wide')
st.title('🥇 XAU/USD SMC Intraday Engine V10')
st.caption('15m bias → 5m SMC setup → 1m execution • manual execution only')

with st.sidebar:
    st.header('Risk & data settings')
    ...
    allow_proxy=st.checkbox('Allow GC futures proxy (research only)',value=False)
    run_validation=st.checkbox('Run validation (slower)',value=False)
    if st.button('🔄 Refresh data now'):
        st.cache_data.clear(); st.rerun()
    st.info('No broker connection or order placement. Verify your broker contract/tick specifications before using position size.')

with st.spinner('Loading market data…'):
    data=load_all(allow_proxy)


# Engine call is kept separate so data errors never produce a blank app.
try:
    result=analyze(data,rr2,news_hold,balance,risk_pct,contract_size,lot_step,min_lot,spread,slippage,run_validation=run_validation)
except Exception as e:
    st.error('Analysis engine stopped safely. No trade plan was produced.')
    st.exception(e)
    st.stop()
...
if result['trade_plan'] and result['trade_plan'].get('valid'):
    ...
    elif result['setup_label']=='RESEARCH ONLY': st.warning('Research-only setup: a futures proxy is being used. Do not copy levels to spot/CFD.')
    else: st.success(f"{result['direction']} confirmed. Wait for the closed 1m confirmation candle and execution-zone retracement.")
    c=st.columns(7)
    ...
    if all(not data.get(tf, pd.DataFrame()).empty for tf in ['5m','1m']):
        a,b=st.columns(2)
        for col,label,key in [(a,'LONG WATCH ZONE','long'),(b,'SHORT WATCH ZONE','short')]:
            lo,hi,typ=result['triggers'][key]; col.metric(label,f'{lo:.2f} – {hi:.2f}'); col.caption(f'{typ}. NOT an entry; wait for the complete sequence.')
...
with tabs[0]:
    ...
    df=data.get(tf)
    if df is None or df.empty: st.info(f'No usable {tf} data. The app is still healthy; the feed did not return that interval.')
    else:
        ...
        for key,label in [('bull_fvg','Bull FVG'),('bear_fvg','Bear FVG'),('bull_ob','Bull OB'),('bear_ob','Bear OB')]:
            z=s.get(key)
            if z: fig.add_hrect(y0=z['low'],y1=z['high'],opacity=0.10,line_width=1,annotation_text=label)
...
with tabs[1]:
    for tf in ['15m','5m','1m']:
        s=result['structure'][tf]; st.subheader(tf); st.write({'Bias':s['bias'],'Trend score':s['trend_score'],'BOS':s['bos'],'CHoCH':s['choch'],'Liquidity sweep':s['sweep'],'Displacement':s['displacement'],'FVG':s['fvg'],'OB':s['ob'],'Premium/Discount':s['premium_discount'],'Swing high':s['swing_high'],'Swing low':s['swing_low']}); st.caption(' • '.join(s.get('reason',[])))
    st.subheader('Execution checklist')
    for name,ok in result['execution']['checks']: st.write(('✅ ' if ok else '⬜ ')+name)
...
with tabs[3]:
    st.subheader('Closed-bar 15m → 5m → 1m validation')
    if not run_validation:
        st.info('Validation is OFF to keep the live dashboard fast. Turn on “Run validation (slower)” in the sidebar when you want research statistics.')
    else:
        v=result['validation']; st.info(v.get('status','Unknown'))
        if v.get('in_sample'):
            c=st.columns(3)
            for col,title,key in zip(c,['In-sample','Out-of-sample','Combined'],['in_sample','out_of_sample','total']): col.markdown(f'### {title}'); col.json(v[key])
        st.caption(v.get('note',''))
with tabs[4]:
    st.markdown('''### Professional SMC execution rules

**15m:** confirmed swing progression + BOS/CHoCH; EMA is secondary context. RANGE = wait.

**5m:** same-direction liquidity sweep → displacement → BOS/CHoCH → FVG/OB POI.

**1m:** same-direction sweep → displacement → BOS/CHoCH → FVG/OB → retracement.

**Entry:** POI midpoint only after the closed 1m sequence is complete.

**SL:** structural invalidation with ATR buffer. A closed 1m candle through invalidation cancels the setup.

**TP1:** 1R. **BE:** only after TP1 is actually reached. **TP2:** selected R multiple.

**Risk:** balance × risk %. Position size includes estimated spread/slippage and broker lot step/minimum.

**News:** directional pressure and event risk are separate. High-impact risk can block a technically valid setup.

**Data:** XAU spot is required for execution-grade levels. GC=F is research-only.

**Manual only:** the app never logs into your broker or places orders.''')

st.caption(f"Last scan: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')} • Data cache: 120s")
